...
summary(lm(hold ~ rational_hold * condition,
  data = filter(dat_main_long, i_block > 1)))
```
...
dat_prepared <- dat_main_long %>%
  ...
  dplyr::mutate(price_move_from_last_corrected = dplyr::recode(
    ...
    condition = dplyr::recode(
      condition, 'baseline' = -.5, 'probs_shown' = .5),
...
summary(belief_updates_treatment)
```
...
summary(lm(de_diff_12_34 ~ main_condition, data = dat_prepared))

# Using the raw DE values:
summary(lm(de_diff_12_34 ~ main_condition, data = complete_table))  
```
