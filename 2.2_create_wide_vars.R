...
# TODO: Rewrite so it only uses dat_main_long as we don't save the participant
# level files anymore.
...
outcome_table_vars <- c(outcome_table_vars, 'ravens_matrices_score',
    'participant_payoff', 'rational_payoff')
...
demographics_vars <- c(
                     'strategy', 'strategy_random', 'strategy_feeling',
                     'strategy_rational', 'strategy_risk_averse',
                     'strategy_inertia', 'strategy_DE', 'strategy_anti_DE',
                     'age', 'gender',
                     'is_student', 'study_field', 'investment_experience',
                     'purpose', 'engagement', 'interest', 'general_comments',
                     'soep_general', 'soep_drive', 'soep_finance',
...
# DE Measure --------------------------------------------------------------
  de_list <- list()
  de_list$n_sales <- vector(mode = 'numeric', length = n_blocks)
  de_list$n_losses <- vector(mode = 'numeric', length = n_blocks)
  de_list$n_gains <- vector(mode = 'numeric', length = n_blocks)
  de_list$n_sold_losses <- vector(mode = 'numeric', length = n_blocks)
  de_list$n_sold_gains <- vector(mode = 'numeric', length = n_blocks)

  for (block_nr in seq_len(n_blocks)) {
    this_dat <- filter(dat, i_block == block_nr - 1)

    # A "sale" in this measure is selling an asset while holding it or buying one while shorted.
    sales <- with(this_dat, transaction < 0 & hold == 1 | transaction > 0 & hold == -1)
    de_list$n_sales[block_nr] <- sum(sales)
    de_list$n_losses[block_nr] <- sum(this_dat$returns < 0)
    de_list$n_gains[block_nr] <- sum(this_dat$returns > 0)
    de_list$n_sold_losses[block_nr] <- this_dat %>% filter(returns < 0 & sales) %>% nrow()
    de_list$n_sold_gains[block_nr] <- this_dat %>% filter(returns > 0 & sales) %>% nrow()
...
# Trading Indicators ------------------------------------------------------
    ...
    short_pos_streaks <- c()
    this_inv_count <- 0
    for (i_time in seq(nrow(this_dat))) {
      if (this_dat$hold[i_time] != 0) {
        this_inv_count <- this_inv_count + 1
      }
      if ((this_dat$transaction[i_time] != 0 |
          i_time == nrow(this_dat)) & this_inv_count != 0) {
        if (this_dat$hold[i_time] == 1) {
          long_inv_streaks <- c(long_inv_streaks, this_inv_count)
        } else if (this_dat$hold[i_time] == -1) {
...
# Reaction Times -----------------------------------------------------
    rt_table[vpn, str_c('mean_RT_', block_nr)] <- this_dat %>%
        dplyr::select(time_to_order) %>%
        colMeans(na.rm = TRUE)

    rt_table[vpn, str_c('mean_lottery_RT_', block_nr)] <- this_dat %>%
  ...
  # Administratives:
  de_table$main_condition[vpn] <- as.character(this_dat$condition[1])
...
# Indicators from dat_main_long #########################################
complete_table <- dat_main_long %>%
  group_by(participant_code) %>%
  summarise(rational_payoff = sum(rational_complete_earnings)) %>%
  left_join(de_table, by = 'participant_code')
