...
target_moves <- c(which(abs(filtered_df$transaction) == 1), jumps) # Change to != 0 when including jumps
...
```{r correct_trades}
correct_trades_table <- dat_main_long %>%
  ...
  correct_trades_table

  ggplot(correct_trades_table,
    aes(x = trade_type, y = avg_correct_trades)) +
