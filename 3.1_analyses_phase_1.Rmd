...
jumps <- which(abs(dat_main_long$transaction) == 2 &
  dat_main_long$i_block < 2)
target_moves <- c(which(abs(dat_main_long$transaction) == 1 &
  dat_main_long$i_block < 2), jumps)
...
full_shift_df <- dat_main_long %>%
  ...
  ungroup() %>%
  filter(n_blocks == 1, target_shift <= hold_length)
...
ggplot(dat_prepared,
  ...
  # scale_color_manual(
    # values = c('Open' = rgb(0.45, 0.65, 0.63),
    #            'Close' = rgb(0.656, 0.856, 0.836))) +
...
jumps <- which(abs(dat_main_long$rational_trade) == 2 &
  dat_main_long$i_block < 2)
target_moves <- c(which(dat_main_long$rational_trade != 0 &
  dat_main_long$i_block < 2), jumps)

# Jumps get a second entry
moves_after_decision <- tibble(
  original_trade_ix = rep(target_moves, each = 6),
  trade_id = rep(seq_along(target_moves), each = 6),
  target_shift = rep(0:5, length(target_moves)),
  was_jump = c(rep(FALSE, (length(target_moves) - length(jumps)) * 6),
...
full_shift_df <- dat_main_long %>%
  ...
  mutate(original_hold = rational_hold[target_shift == 0],
    ...
    price_normal = case_when(
      ...
      original_hold == 1 & was_jump ~ - price_normal,
      TRUE ~ price_normal),
    trade_direction = if_else(
      original_hold == 0 | was_jump, 'Invest', 'Liquidate'),
...
ggplot(dat_prepared,
  ...
  scale_color_manual(
    values = c('Invest' = rgb(0.45, 0.65, 0.63),
               'Liquidate' = rgb(0.656, 0.856, 0.836))) +
  labs(x = 'Periods since Transaction', y = 'Mean Return',
       color = 'Transaction', group = 'Transaction') +
  theme(text = element_text(size = 16))
    # values = c('Invest' = 'Black', 'Liquidate' = '#888888')) +
