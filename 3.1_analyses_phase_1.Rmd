...
olog_beliefs <- clmm(hold_lead ~ belief_residuals + bayes_prob_up +
  (1 | participant_code),
  data = dat_prepared)
...
derp <- rnorm(-.5, .5, n = 10000)
qplot(pnorm(derp), bins = 80)
```
...
m1 <- lm(belief_diff_bayes_corrected_flipped ~ price_move_from_last_corrected,
  data = dat_prepared)
coeftest(m1, vcovCL(m1, cluster = ~dat_prepared$participant_code))
```
