...
Checking whether there is an interaction in the updating during the first two blocks when correcting for a RL learner. These effects are __effect coded!__ and clustered by participant. Comment out the `updating_type != 'Wrong'` filter to get the results with wrong updates included.
...
dat_prepared <- dat_main_long %>%
  ...
  mutate(price_move_from_last_corrected = recode(
    ...
    position_end_of_last_period = recode(position_end_of_last_period,
      'Loss' = -.5, 'Gain' = .5),
    belief_diff_bayes_corrected_flipped = belief_diff_bayes_corrected_flipped /
      100)
...
summary(lm(de_12 + .78 ~ main_condition, data = complete_table))
```

The uncorrected differences in DE values between first and second phase compared between conditions.
```{r de_conditions_treatments_raw}
...
dat_prepared <- dat_main_long %>%
  ...
  mutate(belief_diff_bayes_corrected =
    belief_diff_bayes_corrected / 100)
